...
const FindJobs = () => {
  ...
  const fetchJobs = async () => {
    ...
    // Constructing the experience filter to account for the "Under 1 Year" case
    let experienceFilter = expVal
      .map((val) => {
        if (val === "Under 1 Year") {
          // For "Under 1 Year", we specifically want to target 0 years of experience
          return "0";
        }
        return val.replace(/[^0-9-]/g, ""); // Removes non-numeric characters except the dash (-)
      })
      .join(",");
  ...
  const filterExperience = (e) => {
    ...
    setExpVal((prevExpVal) => {
      if (prevExpVal.includes(e)) {
        return prevExpVal.filter((el) => el !== e);
      } else {
        return [...prevExpVal, e];
      }
    });
  ...
  useEffect(() => {
    fetchJobs();
  }, [sort, filterJobTypes, expVal, page]);
