...
function ApplyNowDialog({ isOpen, onClose, user, jobId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Ensure user and jobId are available
    ...
    try {
      ...
      const response = await apiRequest({
        url: `users/apply-to-job/${user._id}/${jobId}`,
        method: "PATCH",
        token: user.token, // Use the user's token for authentication
        // Depending on how your apiRequest function is set up, you may need to adjust how you're passing the token
      });
      ...
      if (response && response.success) {
        ...
        // Handle additional logic here, e.g., closing the dialog, refreshing user data, etc.
        onClose();
      } else {
        alert(
          "Failed to apply to job: " + (response.message || "Unknown error")
        );
      }
    } catch (error) {
